...
const s = {
  ...
  revokeBtn: { padding: '6px 16px', fontFamily: "'DM Sans', sans-serif", fontSize: '11px', fontWeight: '500', letterSpacing: '0.06em', textTransform: 'uppercase', color: '#C0392B', backgroundColor: 'transparent', border: '1px solid #C0392B', borderRadius: '2px', cursor: 'pointer' },
  badge: { display: 'inline-block', padding: '3px 10px', borderRadius: '2px', fontFamily: "'DM Sans', sans-serif", fontSize: '10px', fontWeight: '500', letterSpacing: '0.08em', textTransform: 'uppercase' },
...
export default function Admin() {
  ...
  async function handleApprove(request) {
    ...
    if (!error) {
      ...
      setSuccessMsg(`${request.name} approved as curator.`)
      fetchRequests()
      fetchCurators()
  ...
  async function handleRevoke(curator) {
    await supabase.from('curators').update({ approved: false }).eq('id', curator.id)
    setSuccessMsg(`${curator.name}'s access revoked.`)
    fetchCurators()
    setTimeout(() => setSuccessMsg(''), 3000)
  }
            ...
            <tbody>
              {curators.map(c => (
                <tr key={c.id}>
                  ...
                  <td style={s.td}>
                    <span style={{ ...s.badge, ...(c.approved ? s.badgeApproved : s.badgePending) }}>
                      {c.approved ? 'Approved' : 'Pending'}
                    </span>
                  </td>
                  <td style={s.td}>
                    {c.approved && c.role !== 'admin' && (
                      <button style={s.revokeBtn} onClick={() => handleRevoke(c)}>Revoke</button>
                    )}
