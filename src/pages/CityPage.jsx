          ...
          <div style={styles.eventsGrid}>
            {events.map((event) => (
              <Link key={event.id} to={`/events/${event.id}`} style={styles.eventCard}>
                ...
                {event.genre && <span style={styles.eventGenre}>{event.genre}</span>}
              </Link>
            ))}
          </div>
        )
        ...
        places.filter(p =>
          ...
          <div style={styles.eventsGrid}>
            {places
              ...
              .map((place) => (
                <div key={place.id} style={styles.eventCard}>
                  <p style={styles.eventDate}>{place.category}</p>
                  <h2 style={styles.eventTitle}>{place.name}</h2>
                  <p style={styles.eventVenue}>{place.address}</p>
                  {place.google_maps_url && (
                    
                      href={place.google_maps_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={styles.eventGenre}
                    >
                      View on Maps
                    </a>
                  )}
